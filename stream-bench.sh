...
#the number of messages per second to send to be processed (default 1000)
#LOAD=${LOAD:-1000} 
#LOAD=${LOAD:-20}  #too low load message number can affect the result benchmark 
LOAD=${LOAD:-20} 
...
run() {
  ...
  then
    ...
    sleep 10
  elif [ "STORM_TEST" = "$OPERATION" ] || [ "1" = "$OPERATION" ];
  then
    run "START_ZK"
    run "START_REDIS"
    run "START_KAFKA"
    ...
    run "STOP_REDIS"
    run "STOP_ZK"
  # STORM_TEST with ENCRYPTED MESSAGE
  elif [ "ENCRYPT_TEST" = "$OPERATION" ] || [ "2" = "$OPERATION" ];
  then
    run "START_ZK"
    run "START_REDIS"
    run "START_KAFKA"
  ...
  then
    ...
    echo "ENCRYPT_TEST: run storm test (assumes SETUP is done)"
    echo
